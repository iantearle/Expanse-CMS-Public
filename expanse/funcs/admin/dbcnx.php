...
function dbcnx_content() {
	...
	if(is_posting(L_BUTTON_EDIT)) {
		...
		$dbcnx = new mysqli($serverName, $databaseUser, $databasePassword);
		if(!$dbcnx) {
			$sqlhost 	= $CONFIG['host'];
			$sqluser	= $CONFIG['user'];
			$sqlpass 	= $CONFIG['pass'];
			printOut(FAILURE, L_DB_NEW_CNX_FAILED);
		} elseif(!mysqli_select_db($dbcnx, $_POST['db'])) {
			$sqldb = $CONFIG['db'];
...
";
	...
	$db = mysqli_select_db($link, $CONFIG['db']);
	if($link) {
	...
	<div class="row-fluid">
		<div class="span12">
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" name="host" class="formfields" id="host" value="<?php echo $CONFIG['host']; ?>" />
				</div>
			</div>
			<div class="control-group">
				<label for="user" class="control-label"><?php echo L_DB_USERNAME ?></label>
				<div class="controls">
					<input name="user" type="text" class="formfields" id="user" value="<?php echo $CONFIG['user']; ?>" />
				</div>
			</div>
			<div class="control-group">
