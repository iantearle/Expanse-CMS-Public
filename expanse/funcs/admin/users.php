	...
	if(ACTION == 'add') {
					...
					if(!empty($the_username)) {
						...
						if(empty($user)) {
							...
							if($users->SaveNew()) {
								...
								printOut(FAILURE, vsprintf(L_USER_NOT_ADDED, array($users->username, mysqli_error())));
							}
						} else {
		...
		<div class="row-fluid">
			...
			<div class="span4">
				<fieldset id="adminBox">
					...
					<div class="control-group">
						<label for="moderateCheck" class="checkbox">
							<input type="checkbox" name="admin" id="moderateCheck" value="1" checked="checked" />
							<?php echo L_USER_ADMIN_MODERATE ?>
						</label>
					</div>
					<div class="control-group">
		...
		if(!empty($item_id)) {
			...
			if(is_posting(L_BUTTON_EDIT)) {
				$the_username = preg_replace('([^[:alnum:]_])', '',$_POST['username']);
				$the_displayname = preg_replace('([^[:alnum:]_[:space:].])', '',$_POST['displayname']);
				$url = httpify(htmlentities($_POST['url'], ENT_QUOTES));
						...
						if(!empty($the_username)) {
							...
							if(empty($user) || $the_username == $users->username) {
								...
								if($users->Save()) {
									...
									printOut(FAILURE, vsprintf(L_EDIT_FAILURE, array($users->username,mysqli_error())));
								}
							} else {
...
>>>>>>> 325e700e95f305a91d7685ba9c9b19b036d2e24c
						...
						<fieldset id="adminBox">
							...
							<input type="hidden" name="permissions" value="" />
							<label for="adminCheck" class="checkbox">
								<input type="checkbox" name="admin" id="adminCheck" value="1" <?php echo $user->admin == 1 ? 'checked="checked"' : ''; ?> />
								<?php echo L_USER_ADMIN ?>
							</label>
							<?php
							if($user->admin == 1) {
...
>>>>>>> 325e700e95f305a91d7685ba9c9b19b036d2e24c
						...
						<fieldset id="categoryBoxes">
							...
							<label id="section_user_label" for="section_admin_user" class="radio">
								<input type="radio" name="section_admin" id="section_admin_user" value="0" <?php echo $user->section_admin == 0 ? 'checked="checked"' : '' ?> />
								<?php echo L_USER_MODERATOR_USER; ?>
							</label>
							<?php
							foreach($cats as $s) {
								?>
								<label for="perm_<?php echo $s->sectionname ?>" class="checkbox">
									<input type="checkbox" name="permissions[]" id="perm_<?php echo $s->sectionname ?>" value="<?php echo $s->id ?>" <?php echo in_array($s->id, $user->permissions) || $user->admin == 1 ? 'checked="checked"' : ''; ?> />
									<?php echo $s->sectionname ?>
								</label>
								<?php
							}
