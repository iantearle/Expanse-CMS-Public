	...
	function createSummary() {
		...
		foreach($ov_list as $val) {
			if($val->cat_type == 'pages') {
				...
				$ov .= '<li><h3>' . ucwords($val->sectionname) . '</h3><p>'.sprintf(L_OVER_PAGES_ONLINE, $online_items, $total_items).'</p></li>';
				continue;
			}
			...
			$online_items = $Database->Result(0, 'itemcount');
			$ov .= '<li><h3>' . ucwords($val->sectionname) . '</h3><p>'.sprintf(L_OVER_ITEMS_ONLINE, $online_items, $total_items).'</p></li>';
		}
		$ov .= '</ul>';
