...
function add() {
    ...
    if (!empty($uploads['errors']) || !empty($xtra_img_uploads['errors'])) {
       ...
       printOut(FAILURE,sprintf(L_UPLOAD_FAILURE, $messages));
		...
		$items->descr = str_replace(array('&nbsp;','<p><br></p>'), array(' ', ''), $items->descr);
		$items->descr = htmlspecialchars_decode(htmlentities($items->descr, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->admission = str_replace(array('&nbsp;','<p><br></p>'), array(' ', ''), $items->admission);
		$items->admission = htmlspecialchars_decode(htmlentities($items->admission, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->events = str_replace(array('&nbsp;','<p><br></p>'), array(' ', ''), $items->events);
		$items->events = htmlspecialchars_decode(htmlentities($items->events, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->dates = str_replace(array('&nbsp;','<p><br></p>'), array(' ', ''), $items->dates);
		$items->dates = htmlspecialchars_decode(htmlentities($items->dates, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->other = str_replace(array('&nbsp;','<p><br></p>'), array(' ', ''), $items->other);
		$items->other = htmlspecialchars_decode(htmlentities($items->other, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		//Add a subcat
		$this->addSubcat();
        //Save the info
        if ($items->SaveNew()) {
...
function edit() {
    ...
    //Process the files
  	$uploaddir = UPLOADS;
    $uploads = checkFiles($_FILES, $uploaddir, true, '/^additional_images\d/i', $item_id);
	$xtra_img_uploads = checkFiles($_FILES, $uploaddir, true, array('img_main', 'img_thumb'));
    //Check for errors
    if (!empty($uploads['errors']) || !empty($xtra_img_uploads['errors'])) {
        foreach($uploads['errors'] as $val){
    ...
    //Everything looks good
		...
		$items->descr = htmlspecialchars_decode(htmlentities($items->descr, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->admission = str_replace(array('&nbsp;','<p></p>'), '', $items->admission);
		$items->admission = htmlspecialchars_decode(htmlentities($items->admission, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->events = str_replace(array('&nbsp;','<p></p>'), '', $items->events);
		$items->events = htmlspecialchars_decode(htmlentities($items->events, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->dates = str_replace(array('&nbsp;','<p></p>'), '', $items->dates);
		$items->dates = htmlspecialchars_decode(htmlentities($items->dates, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);
		$items->other = str_replace(array('&nbsp;','<p></p>'), '', $items->other);
		$items->other = htmlspecialchars_decode(htmlentities($items->other, ENT_NOQUOTES, 'UTF-8'), ENT_NOQUOTES);

		$this->manage_language_fields($items);
		  ...
		  if(!getimagesize($val['filename'])){
			...
			$items->paypal_handling = (isset($_POST['paypal_handling'])) ? (float) $_POST['paypal_handling'] : 0;
			 if($items->SaveNew()){
			 	...
			 	$results['error'][$items->title] = mysqli_error();
			 }
		  }
	  } else{
			unset($exfiles[$ind]);
...
$items =& $this->items;
?>
<label for="crop_x"><?php echo L_GALLERY_THUMB_X ?></label><input type="text" name="crop_x" value="<?php echo $items->crop_x;?>" class="infields" id="crop_x"  /><br />
<label for="crop_y"><?php echo L_GALLERY_THUMB_Y ?></label><input type="text" name="crop_y" value="<?php echo $items->crop_y;?>" class="infields" id="crop_y"  /><br />
<label for="thumb_w"><?php echo L_GALLERY_THUMB_W ?></label><input type="text" name="thumb_w" value="<?php echo $items->thumb_w;?>" class="infields" id="thumb_w"  /><br />
<label for="thumb_h"><?php echo L_GALLERY_THUMB_H ?></label><input type="text" name="thumb_h" value="<?php echo $items->thumb_h;?>" class="infields" id="thumb_h"  /><br />
<label for="thumb_max"><?php echo L_GALLERY_THUMB_MAX ?></label><input type="text" name="thumb_max" value="<?php echo $items->thumb_max;?>" class="infields" id="thumb_max"  /><br />
<span id="keepThumbSize"><label for="use_default_thumbsize"><?php echo L_GALLERY_THUMB_KEEP_DEFAULT ?></label>
<input type="checkbox" name="use_default_thumbsize" value="1" id="use_default_thumbsize"  <?php echo ($items->use_default_thumbsize == 1) ? 'checked="checked"' : '';?>  /><br /></span>
<?php
}
function doPaypal(){
$items =& $this->items;
...
<fieldset><legend><?php echo L_GALLERY_PP_MORE_OPTIONS; tooltip(L_GALLERY_PP_MORE_OPTIONS, L_GALLERY_PP_MORE_OPTIONS_HELP); ?></legend>
...
if($ok === 1){
?>
<label for="option<?php echo $ok ?>"><?php echo L_JS_OPTION_LABEL ?> <?php echo $ok ?></label><input type="text" name="extraoptions[]" id="option<?php echo $ok ?>" value="<?php echo view($val) ?>" class="formfields"  /><br />
<?php
} else {
?>
<div id="option<?php echo $ok ?>Group"><label for="option<?php echo $ok ?>"><?php echo L_JS_OPTION_LABEL ?> <?php echo $ok ?></label><input type="text" name="extraoptions[]" id="option<?php echo $ok ?>" value="<?php echo view($val) ?>" class="formfields"  /><br /></div>
<?php
}
...
?>
<label for="option1"><?php echo L_JS_OPTION_LABEL ?> 1</label><input type="text" name="extraoptions[]" id="option1" value="" class="formfields"  /><br />
<?php
