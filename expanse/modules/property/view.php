		...
		<div class="row-fluid">
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<input name="title" type="text" class="span12 formfields" id="title" />
					</div>
				</div>
			</div>
			<div class="span6">
		...
		<div class="row-fluid">
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<input name="address" type="text" class="span12 formfields" id="address" />
					</div>

					<label for="county" class="control-label">County</label>
					<div class="controls">
						<input name="county" type="text" class="span12 formfields" id="county" />
					</div>
		...
		<div class="row-fluid">
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<input type="email" class="span12 formfields" id="email" name="email" />
					</div>

					<label for="telephone" class="control-label">Telephone</label>
					<div class="controls">
						<input type="text" class="span12 formfields" id="telephone" name="telephone" pattern="^(((\+44\s?\d{4}|\(?0\d{4}\)?)\s?\d{3}\s?\d{3})|((\+44\s?\d{3}|\(?0\d{3}\)?)\s?\d{3}\s?\d{4})|((\+44\s?\d{2}|\(?0\d{2}\)?)\s?\d{4}\s?\d{4}))(\s?\#(\d{4}|\d{3}))?$" />
					</div>

					<label for="url" class="control-label">Website</label>
					<div class="controls">
						<input type="url" class="span12 formfields" id="url" name="url" />
					</div>
				</div>
			</div>
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<div class="input-prepend">
							...
							<input type="text" class="formfields" id="twitter" name="twitter" pattern="^(\w){1,15}$" />
						</div>
		...
		<div class="descr-well">
			<div class="row-fluid">
				<div class="control-group">
					...
					<div class="controls">
						<textarea name="admission" id="admission" class="span12 descr descr-admission"></textarea>
					</div>
				</div>
			</div>
		</div>
		<div class="descr-well">
			<div class="row-fluid">
				<div class="control-group">
					...
					<div class="controls">
						<textarea name="events" id="events" class="span12 descr descr-events"></textarea>
		...
		<div class="descr-well">
			<div class="row-fluid">
				<div class="control-group">
					...
					<div class="controls">
						<textarea name="dates" id="dates" class="span12 descr descr-dates"></textarea>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="descr-well">
					<div class="row-fluid">
						<div class="control-group">
							...
							<div class="controls">
								<textarea name="other" id="other" class="span12 descr descr-other"></textarea>
							</div>
						</div>
					</div>
				</div>
...
if(EDITING):
	...
	if(EDIT_SINGLE):
		if(!($_SESSION['id'] == 2) || !($_SESSION['id'] == $items->aid)) {
			echo '<div class="alert alert-message alert-danger fade in" data-alert="alert"><a class="close" href="#">×</a><p>You have no permissions to edit this file.</p></div>';
			exit;
		}
		$format = new Format();
	?>
		...
		<div class="row-fluid">
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<input name="address" type="text" class="span12 formfields" id="address" value="<?php echo $items->address;?>" />
					</div>
				</div>
				<div class="control-group">
					<label for="county">County</label>
					<div class="controls">
						<input name="county" type="text" class="span12 formfields" id="county" value="<?php echo view($items->county);?>" />
					</div>
				</div>
				<div class="control-group">
		...
		<div class="row-fluid">
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<input type="email" class="span12 formfields" id="email" name="email" value="<?php echo view($items->email);?>" />
					</div>

					<label for="telephone">Telephone</label>
					<div class="controls">
						<input type="text" class="span12 formfields" id="telephone" name="telephone" pattern="^(((\+44\s?\d{4}|\(?0\d{4}\)?)\s?\d{3}\s?\d{3})|((\+44\s?\d{3}|\(?0\d{3}\)?)\s?\d{3}\s?\d{4})|((\+44\s?\d{2}|\(?0\d{2}\)?)\s?\d{4}\s?\d{4}))(\s?\#(\d{4}|\d{3}))?$" value="<?php echo view($items->telephone);?>" />
					</div>

					<label for="url">Website</label>
					<div class="controls">
						<input type="url" class="span12 formfields" id="url" name="url" value="<?php echo view($items->url);?>" />
					</div>
				</div>
			</div>
			<div class="span6">
				<div class="control-group">
					...
					<div class="controls">
						<div class="input-prepend">
							<span class="add-on">@</span>
							<input type="text" class="formfields" id="twitter" name="twitter" pattern="^(\w){1,15}$" value="<?php echo view($items->twitter);?>" />
						</div>
						<span class="help-block">Enter only your <span class="label label-info">username</span></span>
					</div>
		...
		<div class="descr-well">
			<div class="row-fluid">
				<div class="control-group">
					<label for="admission" class="control-label">Admission</label>
					<div class="controls">
						<textarea name="admission" id="admission" class="span12 descr descr descr-admission"><?php echo ($items->admission !== '') ? view($format->HTML($items->admission)) : ""; ?></textarea>
					</div>
				</div>
			</div>
		</div>
		<div class="descr-well">
			<div class="row-fluid">
				<div class="control-group">
					<label for="events" class="control-label">Events</label>
					<div class="controls">
						<textarea name="events" id="events" class="span12 descr descr descr-events"><?php echo ($items->events !== '') ? view($format->HTML($items->events)) : ""; ?></textarea>
						<?php helpBlock(L_PROPERTY_EVENTS_HELP); ?>
					</div>
				</div>
			</div>
		...
		<div class="descr-well">
			<div class="row-fluid">
				<div class="control-group">
					<label for="dates" class="control-label">Dates</label>
					<div class="controls">
						<textarea name="dates" id="dates" class="span12 descr descr descr-dates"><?php echo ($items->dates !== '') ? view($format->HTML($items->dates)) : ""; ?></textarea>
					</div>
				</div>
			</div>
		</div>
				...
				<div class="descr-well">
					<div class="row-fluid">
						<div class="control-group">
							<label for="other" class="control-label">Other</label>
							<div class="controls">
								<textarea name="other" id="other" class="span12 descr descr descr-other"><?php echo ($items->other !== '') ? view($format->HTML($items->other)) : ""; ?></textarea>
		...
		<div class="row-fluid">
				...
				<?php foreach($itemsList as $ind => $item):
					...
					<div id="item_<?php echo $item->id ?>" title="<?php echo strip_tags($item->descr) ?>">
						...
						<a href="http://bufferapp.com/add" class="btn btn-inverse buffer-add-button" data-text="Find <?php echo $item->title . ' ' .$item->county; echo ($item->twitter) ? ' @'.$item->twitter : ''; ?> on Heritage #GoHeritage" data-count="none" data-url="http://hrt.gs/<?php echo $item->hrtgs ?>" <?php echo ($item->image) ? 'http://s3-eu-west-1.amazonaws.com/uploads.heritage-app.com/static/retina'.$item->image : ''; ?>>Buffer</a>
						<fieldset>
